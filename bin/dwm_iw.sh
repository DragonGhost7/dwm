...
dwm_iw() {
   CONSTATE=$(sudo wpa_cli status | sed -n '/wpa_state/s/^.*=//p')

   case $CONSTATE in
      'COMPLETED')
         CONSSID=$(sudo wpa_cli status | sed -n '/\<ssid\>/s/^.*=//p')
         CONIP=$(sudo wpa_cli status | sed -n '/ip_address/s/^.*=//p')
         CONRSSI=$(sudo wpa_cli signal_poll | sed -n '/AVG_RSSI/s/^.*=//p')
